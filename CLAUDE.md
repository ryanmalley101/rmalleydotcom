...
No test framework is configured. ESLint was only wired up (`eslint.config.mjs`, flat config via `eslint-config-next`) once `/professional/tco-calculator` needed it before going public; it had never been run against the rest of the repo before that, so `npm run lint` currently reports ~180 pre-existing issues elsewhere (mostly `react/no-unescaped-entities` and `react-hooks/set-state-in-effect`) that are real but out of scope for whatever you're working on unless asked. Backend changes go through `amplify/` and require an `ampx sandbox` (local dev) or a deploy (`ampx pipeline-deploy`, run automatically by Amplify Hosting's CI on push) before they take effect — editing `amplify/data/resource.ts` alone does nothing until one of those runs.
...
| `/professional` | own dark theme (slate/violet accents); tool pages themselves render their own fully custom UI on top | none |
...
Tools built for Ryan's day job, kept separate from `/software` since they're work-specific rather than general programming projects. Deliberately even-handed rather than sales collateral: since it's hosted publicly, the TCO calculator in particular is built to be vendor-agnostic rather than a Verkada-vs-Milestone pitch.

- **Video Surveillance TCO Calculator** (`/professional/tco-calculator`), originally ported verbatim from a standalone Verkada-vs-Milestone HTML/CSS/JS file, then generalized into a vendor-agnostic two-solution comparator (`lib/model.ts`'s `computeComparison`) driven by a deployment-model abstraction (`"cloud" | "onprem"`) rather than hardcoded vendor formulas. Any combination (on-prem vs. on-prem, cloud vs. cloud, or one of each) reuses the same cost categories; the wizard has an optional "incumbent" setting (`ScenarioInputs.incumbent`) that zeroes one side's year-0 buildout cost when it represents an already-deployed system (its later hardware refreshes still apply on schedule), otherwise both sides pay their own year-0 cost symmetrically. `fleetHalfLifeYears` and `warrantyYears` are per-solution (`SolutionInputs`), not scenario-level, since each side's camera hardware can have its own reliability/warranty; a decay-driven failure still inside its warranty window costs only replacement labor, not the camera itself. Cloud/hybrid solutions have their own free-form license term/price (not a fixed vendor's pricing tiers) and a `migrationStrategy` toggle (`"connector" | "ripReplace"`): "connector" reuses the existing camera fleet behind an NVR-style appliance (à la Verkada's Command Connector) with cameras swapped only as they fail, while "ripReplace" buys out the whole fleet at year 0 with no appliance. Retention's effect on cloud licensing is a continuous curve (`(days/30)^0.4`, fit against the original tool's discrete 30/60/90/180/365-day tiers) rather than a step function, to match on-prem storage's already-continuous scaling and avoid artificial cost cliffs at each threshold day. `annualEscalationPct` compounds every recurring cost year over year, separate from and on top of NPV discounting. Appliance/server unit counts assume at least one per site, not just enough for total camera count, since a connector/NVR box is physically local. On-prem storage cost also applies a RAID overhead multiplier (`SolutionInputs.raidLevel`, defaults to RAID 1): RAID 0 and no RAID need only the usable capacity, RAID 1 and RAID 10 roughly double the raw capacity bought, and that multiplied ("physical") capacity, not the raw usable figure, is what feeds both the storage cost and the on-prem power-draw estimate. `computeComparison`'s crossover search starts its baseline at whichever year first has a nonzero cost gap (usually year 0, not year 1, since an incumbent's side often starts at $0) and now reports every later sign flip too (`laterCrossoverYears`), not just the first; don't naively simplify the baseline back to starting at year 1. A `Stepper`-driven wizard (`page.tsx`) walks through: comparison shape (with generic strengths/tradeoffs copy, no vendor names), shared scenario variables, naming/configuring each solution (with a color picker per solution), then `ResultsView`, which still exposes every assumption for further editing plus two Chart.js (`chart.js/auto`) charts, a "Share link" button that round-trips the whole scenario through a `?s=`-encoded JSON query param (`lib/shareState.ts`; the address bar stays in sync via a debounced `router.replace`, wrapped in `Suspense` per Next's `useSearchParams()` requirement), and a "Download snapshot" button (`lib/exportSnapshot.ts`, `html2canvas`) that captures the stat tiles and charts as a PNG. This route is the **one place in the app that uses Mantine instead of MUI**, a deliberate one-off per direct request, scoped via its own `layout.tsx` (`@mantine/core/styles.css` imported only here, so it doesn't leak into the rest of the site); don't reach for Mantine elsewhere without checking this was intentional and not copied by habit. `page.tsx`'s `Phase` is `"intro" | "wizard" | "results"`: a dedicated `IntroScreen` (title, one-line pitch, the non-affiliation disclaimer, a single "Start" button) comes before the `Stepper`, since the wizard's own step text turned out to read as a wall of text when it was the very first thing on the page; a shared link skips straight to `"results"` and never shows the intro. All prices/discounts are editable placeholder assumptions, not real quotes, and the tool states outright (intro screen, results footer) that it's unaffiliated with every vendor it names. The tool's primary audience is Verkada people comparing against whatever a prospect already runs, not a neutral third party, so `page.tsx`'s `seedSolution`/`defaultIncumbentFor` pre-fill "Verkada" on the relevant cloud slot, "Milestone XProtect · Axis Communications" on the relevant on-prem slot (`vmsProvider`/`cameraProvider` both set, not just the display name, so the dropdowns themselves show pre-selected), and default that on-prem side to incumbent; `SHAPE_OPTIONS` leads with the on-prem-vs-cloud shape (labeled "Cloud / Hybrid vs. On-Prem"). When a shape has two slots of the same model, only slot A gets the vendor default, slot B stays an open placeholder, comparing a vendor against itself by default wouldn't make sense. These are workflow conveniences (always overridable), not changes to the underlying math, which stays symmetric by default. `/professional/layout.tsx` and `/professional/tco-calculator/layout.tsx` are Server Components purely for their `metadata` export (route-specific title/description, since a shared link should preview as something other than the generic root title); the actual MUI/Mantine theme providers live in sibling `*ThemeClient.tsx` Client Components they render. `/professional/error.tsx` is the only error boundary in the app (none existed anywhere else before this route). `InfoLabel`'s tooltip trigger is a real `<button>` with `events={{hover,focus,touch}}` set on the Mantine `Tooltip`, not just a hover-only icon: keyboard `:focus-visible` and touch both open it, plain scripted `.focus()` calls in tests won't (Chromium doesn't mark those focus-visible) so don't mistake that for a bug when testing. `framerateFps` (per-solution, defaults to 24) is a secondary multiplier on top of `bitrateMbps` in the storage formula (`tbUsable`), not a replacement for it; like bitrate, it only has a computed effect on the on-prem branch (cloud storage is priced into the license, not derived from bitrate/framerate at all). On-prem's analytics fields (`analyticsApplianceCost`, `analyticsSoftwareCost`) get their own labeled "Analytics" sub-section in `AssumptionsPanel` (a `SectionLabel` div spanning the grid), separate from the flat field list, since cloud's equivalent cost is bundled into its license price and it wasn't obvious those two fields existed otherwise. `ScenarioStep`'s and `AssumptionsPanel`'s `SimpleGrid`s use more, narrower columns (`{ base: 2, sm: 4 }` and `{ base: 2, sm: 3 }`) with a `leftSection` icon per field in `ScenarioStep`, values here top out in the tens of thousands, they don't need full-width inputs, and the icons help visually anchor a dense grid. `/professional` and `/professional/tco-calculator` both set `robots: { index: false, follow: false }` in their `metadata` exports (verify via the `<meta name="robots">` tag in the rendered HTML, not just the source, if this ever needs re-checking): deliberately not indexed, not full auth-gated, "dampened" on purpose. Cloud's appliance refresh cadence (`applianceRefreshCycleYears`, `yearsUntilNextApplianceRefresh`) used to be a hardcoded `y % 10 === 0` in the formula with no UI control at all, unlike on-prem's fully-editable equivalent; it's now the same editable two-field pattern as on-prem's `refreshCycleYears`/`yearsUntilNextRefresh`, defaulting to 7 years (was an implicit, un-editable 10). If this file keeps growing, the next TCO-calculator change should probably split this bullet into its own short doc instead of appending another sentence.
