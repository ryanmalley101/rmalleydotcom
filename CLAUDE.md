...
Tools built for Ryan's day job, kept separate from `/software` since they're work-specific rather than general programming projects. Deliberately even-handed rather than sales collateral: since it's hosted publicly, the TCO calculator in particular is built to be vendor-agnostic rather than a pitch for any specific vendor.

- **Video Surveillance TCO Calculator** (`/professional/tco-calculator`) is a guided wizard comparing total cost of ownership between two video-management deployments (cloud vs. on-prem, cloud vs. cloud, or on-prem vs. on-prem), driven by a deployment-model abstraction (`"cloud" | "onprem"` in `lib/model.ts`) rather than hardcoded vendor formulas, and built to be vendor-agnostic and even-handed since it's public. This is also the **one place in the app that uses Mantine instead of MUI**, a deliberate one-off scoped to its own `layout.tsx`; don't reach for Mantine elsewhere without checking this was intentional. `app/professional/tco-calculator/README.md` documents the cost model's assumptions, the wizard's UX defaults, and known un-fixed bias risks in the tool's default numbers in detail; read it before changing `lib/model.ts`, `lib/defaults.ts`, or `page.tsx`.
