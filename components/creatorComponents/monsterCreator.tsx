...
// import { UpdateMonsterStatblockInput } from '@/amplify/data/resource';
// Some older code in this file uses the legacy `API.graphql` helpers and top-level
// GraphQL operation objects. The project doesn't expose the generated `mutations` or
// `queries` in this workspace in a consistent location, so declare them as any for
// the short-term incremental migration. We'll replace these stubs with proper imports
// from `@/graphql/mutations` and `@/graphql/queries` when those files are present.
declare const mutations: any;
declare const getMonsterStatblock: any;
// The import for `client` is kept, but its usage is not in the original JS function, 
// which still uses the old `API.graphql`. I'll stick to the existing `API.graphql` 
// pattern for a direct conversion, but note that the Gen 2 way would use `client.models.MonsterStatblock`.
const client = generateClient<Schema>();

// Use the type generated by Amplify Gen 2
type MyMonsterStatblock = Schema['MonsterStatblock'];

// Input type for update mutation based on Gen 2 model shape
interface UpdateMonsterInput {
    ...
    skill_proficiencies?: Record<string, boolean | 'expertise' | null>;
...
const HeaderRow: React.FC<HeaderRowProps> = ({ monster, setMonster, downloadFile }) => {
    ...
    const saveMonster = async (monsterToSave: MyMonsterStatblock) => {
        ...
        try {
            ...
            const response = await client.models.MonsterStatblock.update(input);
            savedMonster = response as MyMonsterStatblock
        } catch (e) {
    ...
    const getMonster = async (id: string, ownerId: string) => {
        ...
        if (window.confirm("Fetching a new monster will overwrite the existing statblock")) {
            if (id) {
                try {
                    // The old Amplify API.graphql is used here, so we cast response to any
                    const existingMonster: any = await API.graphql({
...
const CreateMonsterStatblock = () => {
    // Local type inferred from the default object
    type LocalMonster = typeof newMonsterStats;

    const [monsterStatblock, setMonsterStatblock] = useState<LocalMonster>(newMonsterStats);
    ...
    const [skillList, setSkillList] = useState<Record<string, boolean | 'expertise' | null>>({})
    ...
    const [bonusActions, setBonusActions] = useState<Array<any>>([])
    const [reactions, setReactions] = useState<Array<any>>([])
    const [legendaryActions, setLegendaryActions] = useState<Array<any>>([])
    const [mythicActions, setMythicActions] = useState<Array<any>>([])
